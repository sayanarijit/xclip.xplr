local function setup(args)
          ...
          messages = {
            {
              BashExecSilently = [===[
                if ]===] .. args.copy_paths_command .. [===[ < "${XPLR_PIPE_RESULT_OUT:?}"; then
                  "$XPLR" -m "LogSuccess: %q" "Copied path(s) to clipboard"
                else
          ...
          messages = {
            {
              BashExecSilently = [===[
                if ]===] .. args.copy_paths_command .. [===[ <<< "${PWD:?}"; then
                  "$XPLR" -m "LogSuccess: %q" "Copied $PWD to clipboard"
                else
                  "$XPLR" -m "LogError: %q" "Failed to copy $PWD"
                fi
              ]===],
            },
            "PopMode",
  ...
  if not args.keep_selection then
    table.insert(
      xplr.config.modes.custom.xclip_copy.key_bindings.on_key.y.messages,
      "ClearSelection"
    )
    table.insert(
      xplr.config.modes.custom.xclip_copy.key_bindings.on_key.p.messages,
      "ClearSelection"
    )
    table.insert(
      xplr.config.modes.custom.xclip_copy.key_bindings.on_key.P.messages,
